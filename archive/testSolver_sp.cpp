...
Eigen::ArrayXXd gN_bottom(const Eigen::ArrayXXd& X, const double yLa) {
    // ∂u/∂n = -uy at bottom, uy = -2y u => gN = 2*yLa * u(x,yLa)
    return (2.0*yLa) * (-X.array() - yLa*yLa).exp();
}
Eigen::ArrayXXd gN_top(const Eigen::ArrayXXd& X, const double yLb) {
    // ∂u/∂n = +uy at top => gN = -2*yLb * u(x,yLb)
    return (-2.0*yLb) * (-X.array() - yLb*yLb).exp();
}
Eigen::ArrayXXd gN_left(const double xLa, const Eigen::ArrayXXd& Y) {
    // ∂u/∂n = -ux at left; ux = -u => gN = +u(xLa,y)
    return (-xLa - Y.array().square()).exp();
}
Eigen::ArrayXXd gN_right(const double xLb, const Eigen::ArrayXXd& Y) {
    // ∂u/∂n = +ux at right; ux = -u => gN = -u(xLb,y)
    return -(-xLb - Y.array().square()).exp();
}
*/
...
{    
    ...
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0]
                  << " {w|nw} METHOD k Nel_x Nel_y [xLa xLb yLa yLb] [BC_SPEC] [CASE_JSON]\n \n";
        return 1;
    }
    ...
    BCspec mBC;                        // defaults D on all sides
    if (argc >= 11) {                 // optional trailing arg
        mBC = parseBCspec(argv[10]);
    }

    // if no case file is given, use default testcase0.json
    std::string case_path = (argc >= 12 ? std::string(argv[11]) : std::string("testcase0.json"));
    DG::JsonCase mCASE(case_path);
    double alpha = mCASE.alpha();  // still constant in your assembly
...
if ((A * u_h).isApprox(b) == false) {
    ...
    std::cout << "Solver method=" <<  "Sparse Direct" << ", package=" << "Eigen intel MKL Pardiso" << "\n";
