...
#include <Eigen/AccelerateSupport> // Have to include this before other eigen
#include <Eigen/Dense>
...
#include "DGError.hpp"
#include "accelerate_lu_wrapper.hpp"
...
{    
    ...
    VectorXd u_h;
    if (method == Method::SIPG && alpha >=0) {
        // SPD -> use Cholesky (LL^T) or LDL^T
        Eigen::AccelerateLLT<SparseMatrix<double>, Eigen::Lower> llt;
        llt.compute(A);
        if (llt.info() == Success) {
            u_h = llt.solve(b);
        } else {
            std::cerr << "LLT failed; falling back to LDLTTPP (symmetric pivoted)\n";
            Eigen::AccelerateLDLTTPP<SparseMatrix<double>, Eigen::Lower> ldlt;
            ldlt.compute(A);
            if (ldlt.info() == Success) {
                u_h = ldlt.solve(b);
            } else {
                std::cerr << "LDLTTPP failed\n";
            }
        }
    } else {
        u_h = accelwrap::solve_with_accelerate_lu(A, b);
    }
     /*
    ...
    if (method == Method::SIPG && alpha >= 0.0){ 
        // SPD -> MKL Cholesky
...
if ((A * u_h).isApprox(b) == false) {
    ...
    //std::cout << "Solver method=" <<  "Sparse Direct" << ", package=" << "Eigen Sparse Direct + Apple -framework accelerate" << "\n";
    std::cout << "Solver method=" <<  "Sparse Direct" << ", package=" << "Eigen Sparse Direct + Apple Accelerate LU" << "\n";
